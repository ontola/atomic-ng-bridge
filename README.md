...
inside it with a session. The published SDK sends the app to nextgraph.net, which
only works for a wallet registered on a public broker such as nextgraph.eu. With a local broker,
build NextGraph's dev mode from its monorepo and run `scripts/ng-hosted-up.sh`: the same flow then
runs entirely on localhost, and it does complete (`docs/upstream-findings.md` 7d). With an embedded engine instead (`?ngengine=worker`), a broker that
